...
estimate_tmle <- function(natural, ratios, delta, positive, weights, cens, bounds) {
	...
	fit2 <- sw(
		glm(natural[i, ]$tmp_hmtp_delta ~ offset(qlogis(delta$dn[i, 1])) + wts2,
				family = "binomial")
		# ,
		# 		weights = wts2)
	)
	...
	ds_eps[, 1] <- plogis(qlogis(delta$ds[j, 1]) + eps2[1] + eps2[2]*wts2)
	dn_eps[, 1] <- plogis(qlogis(delta$dn[j, 1]) + eps2[1] + eps2[2]*wts2)
